...
### 5. CSS Build System
*   **Source:** Edit CSS in `src/styles/` (variables, layout, components, utils).
*   **Build:** Run `npm run build:css` to bundle into `style.css`.
*   **IMPORTANT:** `style.css` is **AUTOGENERATED** - never edit it directly.
