...
4. **Candidate Set**: Recent events (top `REFLECTION_CANDIDATE_LIMIT` = 50).
...
- `source_ids`: Array of evidence memory IDs (event IDs).
...
## GOTCHAS & RULES
- **Reflection Decay**: Reflections older than a threshold get a linear penalty (down to 0.25x).
