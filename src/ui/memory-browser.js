 ...
 * Memory Browser UI
 *
 * Handles memory list rendering, CRUD operations, search/filter, and pagination.
 */
...
import { extractCharactersSet, filterMemories, getPaginationInfo, sortMemoriesByDate } from './helpers.js';
import { refreshStats } from './status.js';
import { renderMemoryEdit, renderMemoryItem } from './templates.js';
...
function populateCharacterFilter() {
    ...
    if (characters.length > 0) {
        const optionsHtml = characters.map((char) => `<option value="${char}">${char}</option>`).join('');
